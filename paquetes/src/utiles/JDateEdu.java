...
//      int lFinAno;
    ...
    public void clear(){
     ...
     /** establece la fecha y hora min y segundos*/
    public void setDate(Calendar poDate) {
        try {
            ...
            JDepuracion.anadirTexto(JDepuracion.mclINFORMACION, this.getClass().getName(), e.toString());
        }
        try {
    ...
    /**
     * establece hora minuto y segundos
     */
    public void setHoraMinSeg(java.sql.Timestamp poTime){
        Calendar loCalendar = new GregorianCalendar();
        loCalendar.setTime(poTime);
        setHoraMinSeg(loCalendar);
    }
    
    /**
     * establece hora minuto y segundos
     */
    public void setHoraMinSeg(java.sql.Time poTime){
        Calendar loCalendar = new GregorianCalendar();
        loCalendar.setTime(poTime);
        setHoraMinSeg(loCalendar);
    }
     ...
     * establece hora minuto y segundos
     */
    public void setHoraMinSeg(Calendar loCalendar){
        setHora(loCalendar.get(loCalendar.HOUR_OF_DAY));
        setMinuto(loCalendar.get(loCalendar.MINUTE));
        setSegundo(loCalendar.get(loCalendar.SECOND));
    }
    ...
    /**devuelve el string en un formato que se puede volver a leer*/
    public String toString() {
        StringBuilder lsBuffer = new StringBuilder(19);
       
        if (mlHora == 0 && mlMinuto == 0 && mlSegundo == 0) {
            lsBuffer.append(JCadenas.msRellenarIzq(String.valueOf(getDia()), "0", 2));
            lsBuffer.append('/');
            lsBuffer.append(JCadenas.msRellenarIzq(String.valueOf(getMes()), "0", 2));
            lsBuffer.append('/');
            lsBuffer.append(getAno());
        } else {
            if ((mlAno == 0 && mlMes == 0 && mlDia == 0)
                    || (mlAno == 1899 && mlMes == 12 && mlDia == 30)) {
                if (mlSegundo == 0) {
                    lsBuffer.append(JCadenas.msRellenarIzq(String.valueOf(getHora()), "0", 2));
                    lsBuffer.append(':');
                    lsBuffer.append(JCadenas.msRellenarIzq(String.valueOf(getMinuto()), "0", 2));
                } else {
                    lsBuffer.append(JCadenas.msRellenarIzq(String.valueOf(getHora()), "0", 2));
                    lsBuffer.append(':');
                    lsBuffer.append(JCadenas.msRellenarIzq(String.valueOf(getMinuto()), "0", 2));
                    lsBuffer.append(':');
                    lsBuffer.append(JCadenas.msRellenarIzq(String.valueOf(getSegundo()), "0", 2));
                }
            } else {
                lsBuffer.append(JCadenas.msRellenarIzq(String.valueOf(getDia()), "0", 2));
                lsBuffer.append('/');
     ...
     * @param psFormato cadena fecha
     */
    public String msFormatear(String psFormato) {
        String lsResult = "";
        if (psFormato.equals("yyyyMMdd")) {
            lsResult =
                    JFormat.msFormatearDouble(mlAno, "0000")
                    + JFormat.msFormatearDouble(mlMes, "00")
                    + JFormat.msFormatearDouble(mlDia, "00");
        } else if (psFormato.equals("ddMMyyyy")) {
            lsResult =
                    JFormat.msFormatearDouble(mlDia, "00")
                    + JFormat.msFormatearDouble(mlMes, "00")
                    + JFormat.msFormatearDouble(mlAno, "0000");
        } else {
            lsResult = JFormat.msFormatearFecha(moDate(), psFormato);
        }
...
//      }
    ...
    public static String msHora(String psTexto) {
        String lsTexto = psTexto;
        if (JDateEdu.isDate(psTexto)) {
            try {
                //es q tiene la fecha
                lsTexto = new JDateEdu(psTexto).msFormatear("HH:mm:ss");
            ...
            if (psTexto.indexOf(':') < 0) {
                if (psTexto.length() > 1) {
                    lsTexto = psTexto.substring(0, 2);
                } else {
                    lsTexto = psTexto;
                }
                if (psTexto.length() > 3) {
                    lsTexto += ":" + psTexto.substring(2, 4);
                } else if (psTexto.length() > 2) {
                    lsTexto += ":" + psTexto.substring(2, 3);
                }
                if (psTexto.length() > 5) {
                    lsTexto += ":" + psTexto.substring(4, 6);
                } else if (psTexto.length() > 4) {
                    lsTexto += ":" + psTexto.substring(4, 5);
                }
            }
        }
        return lsTexto;
