...
//    }
    ...
    public void run() {
        ...
        try {
            ...
            JDepuracion.anadirTexto(JDepuracion.mclINFORMACION,getClass().getName(), lsCadenaEXE);
...
//            }
    ...
    public static void abrirDocJAVA(String psDoc) throws IOException{
        java.awt.Desktop.getDesktop().open(new File(psDoc));
    }
    ...
    public static Thread abrirDocumento(String psDoc) {
        //IMPORTANTE: Desktop.getDesktop().open(file);
        //TB ABRE EL FICHERO, PERO NO TIENE EN CUENTA SI ES EN INTERNET, VOLVERLO A PROBAR
        ...
        if ("linux".equals(sSistemaOperativo) || "unix".equals(sSistemaOperativo)) {
            ...
            //abrir archivo en disco duro propio
            loEjecutar = new JEjecutar(new String[]{"xdg-open", psDoc});

            Thread loThread = new Thread(loEjecutar);
            loThread.start();
            return loThread;
            ...
            //abrir archivo en disco duro propio
            loEjecutar = new JEjecutar(new String[]{"open", psDoc});
            ...
            File loFile = new File(psDoc);
            JEjecutar loEjecutar;
            if(loFile.exists() && loFile.isDirectory()){
                loEjecutar = new JEjecutar(new String[]{"explorer.exe", "\"" + psDoc + "\""});
            }else{
                if (psDoc.toLowerCase().indexOf("http:") >= 0 || psDoc.toLowerCase().indexOf("https:") >= 0) {
                    //abrir archivo por http
                    loEjecutar = new JEjecutar(new String[]{"cmd.exe", "/C", "start " + psDoc.replaceAll(" ", "%20")});
                } else {
                    if (psDoc.substring(0, 2).equals("//")) {
                        //abrir archivo por red local
                        loEjecutar = new JEjecutar(new String[]{"cmd.exe", "/C", "\"" + psDoc + "\""});
                    } else {
                        //abrir archivo en disco duro propio
                        loEjecutar = new JEjecutar(new String[]{"cmd.exe", "/C", "\"" + psDoc + "\""});
                    }
    ...
    private static String getCrearAccesoDirecto(String psTargetPath, String psDirectorioTrabajo, String psLinkPath, String psLinkName, String psIconoLocalizacion){
        ...
        lsVB.append("'Creación del objeto Shell");lsVB.append(((char) 13)); lsVB.append(((char) 10));
        ...
        lsVB.append("'Definimos la localización del fichero LNK");lsVB.append(((char) 13)); lsVB.append(((char) 10));
