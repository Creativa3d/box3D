...
public class JProcesoActualizar extends JProcesoAccionAbstracX {
    ...
    public void mostrarMensaje(String psMensaje) {
        if(moCoordinador.getErrores().size()>0 && mbPresentarErrores){
                ...
                for(int i = 0 ; i < moCoordinador.getErrores().size(); i++){
                    lsErrores.append(moCoordinador.getErrores().get(i).toString());
                    lsErrores.append('\n');
                }
                JMsgBox.mensajeInformacionNoModal(null, lsErrores.toString());
