...
import utilesFX.aplicacion.auxiliares.forms.JPanelAUXILIARES;
import ListDatos.*;
...
public class JT2AUXILIARES  extends JT2GENERALBASEModelo   implements ICargaMasiva {
    ...
    public JT2AUXILIARES(final IServerServidorDatos poServidorDatos, final IMostrarPantalla poMostrarPantalla) {
        ...
        moParametros.setTitulo(JFXConfigGlobal.getInstancia().getTextosForms().getTexto(JTEEAUXILIARES.msCTabla));
...
//                new ColorCZ(Color.red));
    ...
    public void valoresDefecto(final JSTabla poTabla) throws Exception {
        if(poTabla.moList.getModoTabla() == JListDatos.mclNuevo) {
            ((JTEEAUXILIARES)poTabla).valoresDefecto();
        }
    }
...
//            return JFXConfigGlobal.getInstancia().getMostrarPantalla().mostrarBusqueda(loConsulta, loTabla);
            return null;
       }
    ...
    public JListDatos getTablaBase() {
        return new JTEEAUXILIARES(moServer).getList();
    }
}