...
public final class WinRegistryWrapper {
    ...
    static {
        ...
        if (Platform.getOS().equals(Platform.OS.WINDOWS)) {
        	...
        	try {
                ...
                JDepuracion.warning("No se ha localizar y agregar al path la biblioteca java de despliegue 'deploy.jar': " + e);  //$NON-NLS-1$
            }
            ...
            try {
                ...
                JDepuracion.warning("No se ha podido cargar la libreria de despliegue 'deploy.dll': " + e);  //$NON-NLS-1$
            }
        }
    }
    ...
    public static String getString(final int hKey, final String path, final String name) {
        try {
            ...
            JDepuracion.severe("No se ha podido obtener la clave de registro con ruta '" + path  //$NON-NLS-1$
                                                     + "' y nombre '" //$NON-NLS-1$
                                                     + name
                                                     + "', se devolvera null: " //$NON-NLS-1$
                                                     + e);
    ...
    public static boolean setStringValue(final int hKey, final String path, final String name, final String value) {
        try {
            ...
            JDepuracion.severe("No se ha podido establecer la clave de registro con ruta '" + path //$NON-NLS-1$
                                                     + "' y nombre '" //$NON-NLS-1$
                                                     + name
