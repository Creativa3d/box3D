...
public class JPanelEDICIONGENERICOJSTable extends JPanelEdicionAbstract {
    private JSTabla moDatos;
    private IPanelControlador moPadre;
    
    /** Creates new form JPanelZONAS */
    public JPanelEDICIONGENERICOJSTable() {
        super();
        initComponents();
...
//        setOpaque(false);
    ...
    public void setDatos(JSTabla poDatos, IPanelControlador poPadre) throws Exception {
        moDatos = poDatos;
        moPadre = poPadre;
    }
    ...
    public String getTitulo() {
        String lsTitulo="";    
        if(moDatos.moList.getModoTabla() == JListDatos.mclNuevo) {
            lsTitulo=moDatos.moList.msTabla + " [Nuevo]" ;
        }else{
            lsTitulo=moDatos.moList.msTabla + " [Edicion]";
        }
        return lsTitulo;
    }
...
//                utilesGUI.tabla.IComponentParaTabla loValor = (utilesGUI.tabla.IComponentParaTabla)jPanel1.getComponent(i);
                ...
                if(moDatos.getFields().get(Integer.valueOf(loComp.getName()).intValue()).getPrincipalSN()){
                    loComp.setEnabled(moDatos.moList.getModoTabla() == JListDatos.mclNuevo);
                }
            }catch(Exception e){
    ...
    public void establecerDatos() throws Exception {
        ...
        moDatos.validarCampos();
    }
    
    public void aceptar() throws Exception {
        int lModo = moDatos.moList.getModoTabla();
        IResultado loResult=moDatos.update(true);
        if(loResult.getBien()){
            //notificamos al padre
            IFilaDatos loFila = (IFilaDatos)moDatos.moList.moFila().clone();
